...
# --- 2026-08-18 WS2 (veri kalitesi) — "q" alanı ---------------------------


def test_unified_prediction_quality_defaults_to_none() -> None:
    ...
    assert prediction.quality is None


def test_parse_quality_whitelist_info_and_meaningless() -> None:
    info = _parse_with_raw(
        '[{"i": 0, "s": "NÖTR", "sc": 0.0, "c": "kargo", "cc": 0.9, '
        '"q": "info"}]'
    )
    assert info[0].quality == "informational"

    mean = _parse_with_raw(
        '[{"i": 0, "s": "NÖTR", "sc": 0.0, "c": "kargo", "cc": 0.9, '
        '"q": "mean"}]'
    )
    assert mean[0].quality == "meaningless"


def test_parse_quality_ok_missing_and_garbage_is_none() -> None:
    ok = _parse_with_raw(
        '[{"i": 0, "s": "NÖTR", "sc": 0.0, "c": "kargo", "cc": 0.9, '
        '"q": "ok"}]'
    )
    assert ok[0].quality is None

    missing = _parse_with_raw(
        '[{"i": 0, "s": "NÖTR", "sc": 0.0, "c": "kargo", "cc": 0.9}]'
    )
    assert missing[0].quality is None

    garbage = _parse_with_raw(
        '[{"i": 0, "s": "NÖTR", "sc": 0.0, "c": "kargo", "cc": 0.9, '
        '"q": "cop_deger"}]'
    )
    assert garbage[0].quality is None

    explicit_null = _parse_with_raw(
        '[{"i": 0, "s": "NÖTR", "sc": 0.0, "c": "kargo", "cc": 0.9, '
        '"q": null}]'
    )
    assert explicit_null[0].quality is None


class _QualityStubEngine:
    """quality_sink pipeline testi için q alanı dolu sahte motor."""
    ...
    async def classify_unified_batch_async(
        ...
        *,
        ...
        qualities = ["informational", None, "meaningless"]
        predictions = [
            UnifiedPrediction(
                sentiment_label="NÖTR",
                sentiment_score=0.0,
...
@pytest.mark.asyncio
async def test_pipeline_fills_quality_sink() -> None:
    pipeline = AnalysisPipeline(analyzer=_NeverCalledAnalyzer())
    sink: list[str | None] = []
    await pipeline.analyze_batch_unified_async(
        ["Paketiniz dağıtıma çıktı", "Normal yorum", "asdkfj"],
        engine=_QualityStubEngine(),  # type: ignore[arg-type]
        available_categories=["belirsiz"],
        quality_sink=sink,
    )
    assert sink == ["informational", None, "meaningless"]
